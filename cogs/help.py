...
class Help(Cog):
    ...
    async def help(self, ctx):
        main = Embed(
            title="📔 Help Menu",
            description=f"• All commands are slash commands.\n• Detailed description of every command can be found while executing it.",
            color=Color.blurple(),
        )
        ...
        for command in self.bot.slash_commands:
            ...
            if command.cog.description not in [x.title for x in embeds]:
                e = Embed(
                    title=command.cog.description, description="", color=Color.blurple()
                )
